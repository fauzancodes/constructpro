...
export const GetAllAboutUs = async (
  ...
  if (search) {
    whereCondition.OR = [
      ...
      { callToAction: { contains: search, mode: "insensitive" } },
    ];
  }

  try {
    const totalFiltered = await prisma.about.count({
      where: whereCondition
    });
    const totalData = await prisma.about.count({
      where: {
        deletedAt: null
      }
    });
    ...
    const data = await prisma.about.findMany(queryOptions);
...
export const CreateAboutUs = async (createData: AboutUs) => {
  try {
    const newData = await prisma.about.create({
      data: {
        title: createData.title || null,
        description: createData.description || null,
        callToAction: createData.callToAction || null,
...
export const UpdateAboutUs = async (updateData: AboutUs) => {
  try {
    const updatedData = await prisma.about.update({
      ...
      data: {
        ...
        callToAction: updateData.callToAction || null,
...
export const GetAboutUsById = async (id: string) => {
  try {
    const datasets = await prisma.about.findUnique({
...
export const DeleteAboutUs = async (id: string) => {
  try {
    const deleted = await prisma.about.update({
      where: {
        id,
      },
      data: {
